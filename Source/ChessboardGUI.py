...
class KChessboardGUI:
    ...
    def make_board(self, graph):
        for i in range(1, self.k):
            graph.DrawLine((0, self.boardSize / self.k * i),
                           (self.boardSize, self.boardSize / self.k * i))
            graph.DrawLine((self.boardSize / self.k * i, 0),
                           (self.boardSize / self.k * i, self.boardSize))

        for i in range(round(self.k / 2)):
            for j in range(self.k):
                graph.DrawRectangle(
                    ((self.boardSize / round(self.k / 2) * i + self.boardSize / self.k * j) % self.boardSize,
                     self.boardSize / self.k + self.boardSize / self.k * j),
                    (self.boardSize / self.k + (self.boardSize / round(self.k / 2) * i + self.boardSize / self.k * j) % self.boardSize,
